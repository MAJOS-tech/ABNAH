...
function renderExplorer() { const outlet = document.querySelector("#filter-outlet").value, menu = document.querySelector("#filter-menu").value, vendor = document.querySelector("#filter-vendor").value; const list = (target, rows, left, right) => document.querySelector(target).innerHTML = rows.map(row => `<p class="line"><b>${html(left(row))}</b><span>${right(row)}</span></p>`).join("") || "No rows match this filter."; list("#explorer-menu", (liveData.menu || []).filter(row => (!outlet || row.store === outlet) && (!menu || row.menu_item === menu)), row => `${row.menu_item} · ${row.store}`, row => `${rupees(row.gross_margin)} · ${num(row.gross_margin_pct).toFixed(2)}%`); list("#explorer-vendors", (liveData.procurement || []).filter(row => (!outlet || row.store === outlet) && (!vendor || row.vendor_name === vendor)), row => row.vendor_name, row => `${row.store} · ${rupees(row.open_liability)}`); list("#explorer-outlets", (liveData.outlets || []).filter(row => !outlet || row.store === outlet), row => row.store, row => `${rupees(row.net_sales)} sales · ${num(row.gross_margin_pct).toFixed(2)}% GM`);
  const recipes = (liveData.recipe || []).filter(row => menu && row.menu_item_name === menu); const traces = liveData.vendorTrace || [];
  document.querySelector("#explorer-trace").innerHTML = menu ? recipes.map(recipe => { const sources = traces.filter(trace => trace.menu_item_name === menu && trace.ingredient_name === recipe.ingredient_name && (!outlet || trace.store === outlet) && (!vendor || trace.vendor_name === vendor)); const source = sources[0]; return `<tr><td><b>${html(recipe.ingredient_name)}</b></td><td>${html(recipe.qty_per_menu_unit)} ${html(recipe.canonical_uom)}</td><td>${source ? `${html(source.vendor_name)}<br><small>${html(source.po_number)}</small>` : "No open supplier PO"}</td><td>${source ? html(source.store) : "—"}</td><td>${source ? rupees(source.open_liability) : "—"}</td><td>${source ? `<span class="pill ${String(source.risk_color || "").toLowerCase()}">${html(source.risk_color || "Tracked")}</span>` : "—"}</td></tr>`; }).join("") || "<tr><td colspan='6'>Choose a menu item to show its full ingredient trace.</td></tr>" : "<tr><td colspan='6'>Choose a menu item to show its full ingredient trace.</td></tr>";
}
